...
public class FilesController {
    ...
    @Operation(summary = "Upload files")
    @PostMapping(value = "/upload_cccd", consumes = "multipart/form-data")
    public ResponseEntity<Map<Integer, String>> uploadFileCCCD(@RequestPart("file") MultipartFile file) {
        storageService.init();
        Map<Integer, String> res = new HashMap<>();
        try {
            res = storageService.saveCCCD(file);
            return ResponseEntity.ok(res);
        } catch (Exception e) {
            res.put(0, "Có lỗi xảy ra trong quá trình upload! " + e.getMessage());
            return ResponseEntity.ok(res);
